...
import corrupt from '/Corrupt.svg'
import fill from '/fill.svg'
import cheese_dirt from '/Cheese_dirt.svg'
import cheese_grass from '/Cheese_grass.svg'
import cheese_grass_light from '/Cheese_grass_light.svg'
import op from '/spin-thing.svg'
...
function Block(props){
  ...
  const P_pos = useContext(pos)
  const R_pos = {x: props.x-P_pos.x, y: -props.y+P_pos.y+1-props.h}
  useEffect(() => {
      ...
      {blockRenderList.map((block) => {
        ...
        switch (block.type) {
          ...
          case "cheese-grass":
            ...
            break;
          case "cheese-grass-light":
            src = cheese_grass_light;
            if(memoizedImg.current[src]){
              if(cacheItems.current.includes(src)){
                src_ = src
              }else{
        ...
        return (
          <Block key={`${block.x}:${block.y}`} promise={src_} x={block.x} y={block.y} w={w} h={h}/>
        )
      })}
    </>
  )
...
function Game({ ref, upload }){
  ...
  const [engineList, setEngineList] = useState(() => {
    if (host) {
      __default__[0] = data.seed;
      __default__[6] = data.scale;
      __default__[7] = data.vip;
    }
    return __default__;
  });
